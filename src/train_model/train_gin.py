    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(model.state_dict(), f'best_gin_model_fold_{fold + 1}.pt')
        ...
        np.savetxt(f'gin_train_loss_fold_{fold + 1}.txt', train_losses)
        np.savetxt(f'gin_val_loss_fold_{fold + 1}.txt', val_losses)
...
def main():
    ...
    print("Starting optimized GIN training with data augmentation...")
    ...
    fold_df.to_csv(f'gin_fold_results_{timestamp}.csv', index=False)

    ### Save summary
    summary_df = pd.DataFrame([results_summary])
    summary_df.to_csv(f'gin_training_summary_{timestamp}.csv', index=False)
